...
- `export-tv-watchlist`: use to export `Trading Proposals` for one `run_id` to a TradingView watchlist `.txt` file (`YYYY-MM-DD-<run_id>.txt`) with TV symbol resolution via `symbol_search/v3` (read-only Notion; local file write only).
...
- `fastio-cli`: use for basic Fast.io cloud file operations (list, create folders, upload, download, search) and Trading Proposals session storage; resolves workspace and folders by name (`FASTIO_WORKSPACE_NAME`, `trading-proposals/sessions/`).
