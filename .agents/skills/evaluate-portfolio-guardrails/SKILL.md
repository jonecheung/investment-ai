...
description: Compute portfolio heat, concentration, cash, and exposure metrics from the latest Approved Portfolio Snapshot in Notion and check against active Portfolio Policy hard limits. Read-only. Use when evaluating portfolio guardrails, portfolio heat, or snapshot metrics against Portfolio Policy.
disable-model-invocation: true
---

# Evaluate Portfolio Guardrails

Compute portfolio metrics from the **latest Approved Portfolio Snapshot** and its **Portfolio Holdings** in Notion, then check hard limits from the active **`Portfolio Policy`** row in Notion.
...
- Policy: active Notion **`Portfolio Policy`** row (`Active = true`, else latest `Status = active`)
- Policy override: optional `--policy data/portfolio/guardrails.yaml` for local YAML instead of Notion
...
- Notion databases: `Portfolio Snapshot`, `Portfolio Holdings`, `Portfolio Policy` with schema per [`data/notion/portfolio.md`](../../data/notion/portfolio.md) and [`data/portfolio/guardrails.md`](../../data/portfolio/guardrails.md)
- At least one **Approved Portfolio Snapshot** row exists
- At least one active **`Portfolio Policy`** row exists (unless using `--policy` YAML override)
...
| Flag | Purpose |
| --- | --- |
| `--policy` | Optional YAML policy override (default: active Notion `Portfolio Policy`) |
...
1. Load guidance:
   ...
   - Read [`data/portfolio/guardrails.md`](../../data/portfolio/guardrails.md) (Notion policy schema)
...
4. Summarize results for the user:
   ...
   - Active policy title / effective date (from `guardrail_check.policy_source`)
...
| `guardrail_check` | Policy source, effective limits, checks, summary |
| `warnings` | Optional portfolio-level warnings |

### `guardrail_check.policy_source`

| `type` | Fields |
| --- | --- |
| `notion` | `id`, `title`, `effective_date` |
| `yaml` | `path` |
...
- `Portfolio Snapshot`, `Portfolio Holdings`, or `Portfolio Policy` data source not found
- Required Notion properties are missing
- No approved snapshot exists (or no match for `--snapshot-date`)
- No active `Portfolio Policy` exists (unless `--policy` YAML override is used)
- NAV is missing or zero
- Policy YAML cannot be loaded (when `--policy` is used)
...
- YAML template: [`data/portfolio/guardrails.yaml`](../../data/portfolio/guardrails.yaml)
